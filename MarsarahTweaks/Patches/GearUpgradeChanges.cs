		...
		class GearUpgradeUnlock_Patch
		{
			static void Postfix(ref ObjectDB __instance)
			{
				...
				if (ZNet.instance != null)
				{
					...
					{
						MarsarahTweaks.MLog($"ObjectDB Awake: Updating {ConfigManager.Configs.GearUpgradeModifications.Name}...");

						UpdateGearRecipeUnlock(__instance, false);
					}
					else
					{
						MarsarahTweaks.MLog($"ObjectDB Awake: I am a server. No changes made to {ConfigManager.Configs.GearUpgradeModifications.Name}...");
					}
				}
				else
				{
					MarsarahTweaks.MLog($"ObjectDB Awake: Too early to do anything. No changes made to {ConfigManager.Configs.GearUpgradeModifications.Name}...");
				}
			}
		}
