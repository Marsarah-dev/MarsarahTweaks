...
namespace MarsarahTweaks.Patches
{
	internal class GearRecipeChanges
	{
		...
		{
			// newGearRecipesAmounts Dictionary ============================================
			var newGearRecipesAmounts = new Dictionary<string, Dictionary<string, (int? amount, int? amountPerLevel)>>()
			{
			...
			var newGearRecipesMaterials = new Dictionary<string, Dictionary<string, (string newResItem, int? amount, int? amountPerLevel)>>()
			{
				{
			...
			foreach (Recipe recipe in objDB.m_recipes)
			{
				...
				foreach (Piece.Requirement req in recipe.m_resources)
				{
					...
					if (hasGearMaterialsChange && newGearRecipesMaterials[recipe.name].TryGetValue(req.m_resItem.name, out var materialValues))
					{
